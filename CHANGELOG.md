...
- `rename_directory` and `move_directory` no longer corrupt the recorded paths of documents in *sibling* directories that share a name prefix. The batch path update matched with an unanchored, unescaped SQL `LIKE`, so renaming `/data/docs` also rewrote paths under `/data/docs2`, and `_` in a directory name acted as a single-character wildcard (renaming `/data/my_dir` rewrote `/data/myxdir`). Matching is now anchored at the directory boundary with `LIKE` wildcards escaped, in both the SQLite store (paths and document roots) and the Qdrant index path updater.
