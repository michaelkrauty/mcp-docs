...
class TestRenameDirectory:
    ...
    async def test_rename_directory_invalid_name(
        ...
        assert "path separators" in result["error"]["message"]
...
class TestDeleteDirectory:
    ...
    async def test_delete_directory_has_files(
        ...
        assert "error" in result
        assert result["error"]["code"] == ErrorCode.CONFLICT.value
        assert "contains files" in result["error"]["message"]
    ...
    async def test_delete_directory_is_root(
        ...
        assert "error" in result
        assert result["error"]["code"] == ErrorCode.CONFLICT.value
        assert "document root" in result["error"]["message"]
    ...
    async def test_delete_directory_has_registered_docs(
        ...
        assert "error" in result
        assert result["error"]["code"] == ErrorCode.CONFLICT.value
        assert "registered documents" in result["error"]["message"]
...
class TestValidationHelpers:
    ...
    async def test_path_outside_document_root(
        ...
        assert "not in document root" in result["error"]["message"]
    ...
    async def test_relative_path_error(
        ...
        assert "error" in result
        assert result["error"]["code"] == ErrorCode.INVALID_INPUT.value
        assert "must be absolute" in result["error"]["message"]
