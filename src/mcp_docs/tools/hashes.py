"""Hash verification tools for mcp-facts integration.
...
async def lookup_hash(content_hash: str) -> dict:
    ...
    Used by mcp-facts to resolve document references stored as hashes.
    ...
    if result is None:
        return error_response(ErrorCode.NOT_FOUND, f"No document found with hash: {content_hash[:16]}...")
