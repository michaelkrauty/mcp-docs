...
async def move_file(source_path: str, destination_path: str) -> dict:
    ...
    try:
        ...
        if not dest_parent.exists():
            return error_response(ErrorCode.INVALID_INPUT, f"Destination directory does not exist: {dest_parent}")
        ...
        if not source_valid:
            return error_response(ErrorCode.PERMISSION_DENIED, f"Source path not in document root: {source_root}")
        
        dest_valid, dest_root_result = _validate_within_root(dest, store)
        if not dest_valid:
            return error_response(ErrorCode.PERMISSION_DENIED, f"Destination path not in document root: {dest_root_result}")
        ...
        if not processing_complete:
            return error_response(ErrorCode.TIMEOUT, f"Document processing did not complete within timeout")
...
async def create_directory(path: str, parents: bool = False) -> dict:
    ...
    try:
        ...
        if not valid:
            return error_response(ErrorCode.PERMISSION_DENIED, f"Path not in document root: {root_or_error}")
        ...
        if dir_path.exists():
            if dir_path.is_dir():
                ...
                return error_response(ErrorCode.CONFLICT, f"Path exists but is not a directory: {dir_path}")
...
async def rename_directory(path: str, new_name: str) -> dict:
    ...
    try:
        ...
        if "/" in new_name or "\\" in new_name:
            return error_response(ErrorCode.INVALID_INPUT, "New name must not contain path separators")
...
async def move_directory(source_path: str, destination_path: str) -> dict:
    ...
    try:
        ...
        if not source.exists():
            return error_response(ErrorCode.FILE_NOT_FOUND, f"Source directory does not exist: {source}")
        ...
        dest_parent = dest.parent
        if not dest_parent.exists():
            return error_response(ErrorCode.INVALID_INPUT, f"Destination parent does not exist: {dest_parent}")
    ...
    try:
        ...
        for root in roots:
            if Path(root.path).resolve() == dir_path:
                return error_response(ErrorCode.CONFLICT, f"Cannot delete document root: {dir_path}")
